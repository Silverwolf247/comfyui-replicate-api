...
Delivering the result: embed EVERY output image in your reply with markdown image syntax — `![short description](url)` — never a bare link (a link renders as text and the user never sees the picture). Mention that the URL expires in about 1 hour. Do not download the image bytes.
