...
Always use exactly that version id. Then poll get-prediction every 3-5s until status is succeeded or failed; on success return the output image URLs to the user as-is (they expire in ~1 hour; never download the bytes).
